...
def draw_boxed_text(text, x, y, text_color=(30, 50, 30), bg=(180, 240, 200, 220)):
...
def draw_drag_bubble(item_img, mouse_pos):
    # petite bulle pastel avec l’item dedans
    size = 80
...
def main():
    ...
    clients = []
    client_imgs = []
    client_positions = [(80, 200), (285, 200), (490, 200)]  # x, y
    ...
    while running:
        ...
        for i, client in enumerate(clients):
            ...
            draw_boxed_text(client.request, x + 40, y - 70)
                ...
                if dragging_item:
                    ...
                    for i, rect in enumerate(client_rects):
                        if rect.collidepoint(pos):
                            if dragged_item_name == clients[i].request:
                                score += 1
                                gain = 5 + 2 * upgrades["stock"]["level"]
                                money += gain
                                result_message = "good"
        ...
        if shop_message:
            if pygame.time.get_ticks() - shop_message_timer < 1200:
                draw_boxed_text(shop_message, 260, 520)
            else:
                shop_message = ""
